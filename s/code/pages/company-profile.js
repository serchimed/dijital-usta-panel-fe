  ...
  $tbody.addEventListener("tableLoaded", (e) => {
    ...
    rows.forEach((tr, index) => {
      ...
      if (item && item.memberId) {
        ...
        let $interviewBtn = createInterviewReportButton(item.memberId, USER.companyId, item.displayName, item.isShortlisted || true, $hireBtn, item.isInterviewResulted, item.isHired);
        let $shortlistBtn = createShortlistButton(item.memberId, USER.companyId, item.displayName, item.isShortlisted || true, $msg, $interviewBtn, item.isInterviewResulted, item.isHired);
