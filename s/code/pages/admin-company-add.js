onAuthReady(() => {
  ...
  $btn.addEventListener(CLICK_EVENT, async function () {
    ...
    if (req.phone && !checkPhone(req.phone)) {
      errors.push("Geçerli bir telefon numarası giriniz (0 ile başlayan 11 haneli, örn: 05556667788).");
    }

    if (req.webUrl && !checkUrl(req.webUrl)) {
      errors.push("Geçerli bir web sitesi URL'si giriniz.");
    }

    if (req.trendyolUrl && !checkUrl(req.trendyolUrl)) {
      errors.push("Geçerli bir Trendyol profil URL'si giriniz.");
    }

    if (req.driveUrl && !checkUrl(req.driveUrl)) {
